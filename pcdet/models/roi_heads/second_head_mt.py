...
class SECONDHeadMT(RoIHeadTemplate):
    ...
    def forward(self, batch_dict):
        ...
        if 'detection_loss' in batch_dict:
            targets_dict['detection_loss'] = batch_dict['detection_loss']
    ...
    def get_box_iou_layer_loss(self, forward_ret_dict):
        ...
        if 'detection_loss' in forward_ret_dict:
            # print(forward_ret_dict['detection_loss'])
            rcnn_loss_iou = (batch_loss_iou * iou_valid_mask * forward_ret_dict['detection_loss'].unsqueeze(-1).unsqueeze(-1)).sum() / torch.clamp(iou_valid_mask.sum(), min=1.0)
        else:
