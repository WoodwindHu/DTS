...
def save_beam_labels(frame, cur_save_path, use_two_returns=True):
    range_images, camera_projections, range_image_top_pose = \
        frame_utils.parse_range_image_and_camera_projection(frame)

    beam_labels = get_beam_labels_from_range_image(
        frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1) if use_two_returns else (0,)
    )
    beam_labels_all = np.concatenate(beam_labels, axis=0).astype(np.float32)
    np.save(cur_save_path,beam_labels_all)
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True):
    ...
    for cnt, data in enumerate(dataset):
        ...
        save_beam_labels(
                frame, cur_save_dir / ('%04d.beam_labels.npy' % cnt)
            )
...
def get_beam_labels_from_range_image(frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1)):
    """
    Modified from the codes of Waymo Open Dataset.
    Get beam labels from range image.
    Args:
        frame: open dataset frame
        range_images: A dict of {laser_name, [range_image_first_return, range_image_second_return]}.
        camera_projections: A dict of {laser_name,
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            range_image_tensor = tf.reshape(
                tf.convert_to_tensor(range_image.data), range_image.shape.dims)
            range_image_mask = range_image_tensor[..., 0] > 0
            # print(range_image_mask.shape.dims)
            range_image_beam_label = tf.repeat(tf.expand_dims(tf.range(range_image_mask.shape.dims[0]),1),[range_image_mask.shape.dims[1]],axis=1)
            beam_label_tensor = tf.gather_nd(range_image_beam_label, tf.compat.v1.where(range_image_mask))
            
            beam_label_single.append(beam_label_tensor.numpy())

        beam_label.append(np.concatenate(beam_label_single, axis=0))
